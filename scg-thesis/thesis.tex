...
\begin{quotation}
\noindent 
For the computer to use and understand data, it is necessary to parse it into a computer readable format. The building of a parser requires a model of the grammar and the structure of the data must be known. For many new programming language dialects and also for log files of new software, these models are \chg{normally}{sometimes} not available. For creating a model, the structure and grammar information must therefore be inferred from the source code, which is a time consuming process. The goal of this project is\del{, helping developers,}\ins{ to} find structure of software code automatically. We introduce in this thesis a clustering approach with k-means, for \chg{these purposes}{this purpose}. Through different vector representations, structural information should be exploited and assigned accordingly in different clusters. This should help developers get a better overview of the data and in a perfect case, make them able to create parser rules for each cluster.

The used approach \chg{had}{has} some promising results, assigning different patterns into different clusters. \ugh{We used overall 24 different possible combinations of representation}\ho{What representation are talking about? A new reader would have no idea what you're talking about}. Of these 24 representation, there were four representations with good results, having a v-measure score equal or higher than $0.66$, from a maximum of 1, for Java, and another 4 with satisfying results, having equal or better scores than $0.4$, also for Java. The Representers with good scores in Java, also did a good job, clustering C Sharp, XML and ABACUS log files, with at least one representation scoring higher than $0.66$. For C++, Python the scores were satisfying. Only for the language Brainfuck, the clustering with these Representers did not work well.\ho{This paragraph is a bit confusing. Explain first what a representation is. Then how the clustering works (cluster lines in files into similar structure clusters) .Then what the experiments you did. Then summarize the results without giving too much details}

\chg{Unfortunately}{However,} our approach for using the k-means implementation for calculating a precision value for how closely languages are related, did not perform well, as it classified every language with the exception of XML, a very good match. \ho{Again, what are you talking about? First say that we tried to use our approach to classify languages \etc then say the results.}
\end{quotation}
\clearpage
...
\section{Motivation}
Writing software means reading software code \cite{latoza2006maintaining}. For developers it is crucial to understand the state of the system in order to carry out development and software evolution tasks. As a consequence, developers often use as much time reading and understanding the written code as actually writing new code. \chg{Analysing}{Analysis} tools for integrated development environments can help developers understanding code and reduce time for assessing software code \cite{nierstrasz2012agile}. \chg{That s}{For s}uch tools \chg{can}{to} be created, a software model of the the language the tool is \chg{build}{built} for, must exist in \ins{the }form of a parser. For many dialects of programming language these models do not exists. Aside from programming languages, log files are a very important part of software applications. Many software applications do produce auxiliary text files as output. These log files are used in various ways, for example in debugging and profiling purposes. While generating log files is a very simple and straightforward process, the understanding of log files can be quite hard, as these can be very large files with complex structure \cite{valdman2001log}.  If a model of the grammar and the underlying structure does not exist, it is necessary to infer these properties from the source code. The inferring of a software model and the building of a custom parser is a complex task that can take several people several months to accomplish. In this bachelors project, we \chg{tried}{try} to build a tool that can\del{ help} distinguish different pattern and structures automatically from software code or log files \cite{dubey2006inferring}.
\section{Goal and Focus}
The goal of this thesis is to develop a software tool that can atomically find structure in (unknown) software code or log files and generate different sets consisting of related pattern\ins{s}, so that a parser rule can be inferred. The process of creating the software tool \chg{manifested}{is divided to} in three different sub-problems that had to be addressed:

\begin{enumerate}[ label=\textbf{\arabic*}.]
	
	\item \textbf{Statement creation.} Structure manifests itself in an ordered sequence of different tokens. For finding different patterns, the smallest statement had to be found which is able to hold structural information. In this thesis, \ugh{it is assumed that carriage returns are a natural and often used way, the beginning of a new pattern is indicated}. A statement is then split into tokens. \ugh{A token consists of one or more symbols and is defined through, first, the encapsulating of blank spaces and second, in a change of symbols}\ho{didn't understand this sentence}.


	\item \textbf{Representation and differentiation.} For algorithmically deciding and quantifying the difference between statements, \chg{a way for mathematically representing}{a mathematical representation of} the statements needs to be introduced. The approaches used in these methods, differ from creating a vector out of defining features, but instead focuses on the type of symbols used in a token.
...
To achieve a solution to these sub-problems, certain assumptions about the commonalities of languages and log files had to be made. We \chg{tried}{try} to keep these assumptions as minimal as possible, so that a wide variety of programming languages and log files could be analysed:
...
\ho{It would be nice to add a subsection called "Outline". As an example, look at \url{http://scg.unibe.ch/archive/masters/Ibra15a.pdf}}

\chapter{Methods}
\label{sec:methods}
In this chapter we describe in detail the used methods for creating and tokenizing the input data and put forward the different representations used in the created tool \ho{It would be better if you give your tool a name and then use this name when you want to refer to your tool. It is better than saying ``my tool" or ``our tool" every time you want to refer to it.}. \ugh{In our context, a method is defined as a process, which transforms the input data and if replaced, would result in a different output of the tool. For visualising our methods, we use the following simplified Java example: }\ho{I didn't get it. Do you mean that you will use this example code as a running example? If yes, then introduce it when you first use it, not here at the beginning of the chapter.}
...
To be able to cluster \ugh{the data}\ho{what data?}, the input data first needs to be processed into smaller instances that are comparable to each other. As mentioned in the introduction, we \chg{used}{use} in this project two instances of segmentation: Statements and tokens, where a statement is consisting of one or more tokens. A statement can be created in different ways. Because the main effort of this thesis lies in the structural clustering using the k-means algorithm, the creation of statements is simplified.
...
In our previous example code, each new line would become a statement, creating three different statements from this code snippet: \ho{The green color you use in your listings is not readable. Please choose anther color.}
\begin{center}
...
\end{lstlisting}
\end{center}
\textbf{A token is a single word, numeral or punctuation.} A statement is further processed into tokens\ins{, which are words, numerals, or punctuations.} \del{the smallest instance Words are a sequence of at least one letter and can contain numerals. }
\begin{itemize}
...
\label{sec:reps}
As mentioned in the introduction, vector representation is needed for clustering statements. \ugh{A representer adds or modifies representation to a statement}\ho{What?}. There are three different categories of representers: Type representers, distance representers and weight representers.
\begin{enumerate}[ label=\textbf{\arabic*}.]
	
		\item \textbf{Type Representer}\del{ Type Representers} create\ins{s} vector representations according to the characteristics of the tokens of the statement and assign numbers to the vector accordingly.
	
	
		\item \textbf{Distance Representer} \del{Distance Representers }calculate\ins{s} distance information from the given representation of a Type Representer.
	
		\item \textbf{Weight Representer} \del{Weight Representers }function\ins{s} as a noise cancelling or filtering mechanism and can weight specific parts of a representation.
...
\subsection{Type Structure Representer}
...
\end{centering}
\ho{It is not clear. Please clarify what is a vector\_element, what is assigned\_token\_value, and what is type\_value. I couldn't make sense of it.}
Example: Same as in the Type Structure Representer we assigned the values 0 for words, 1 for numerals and 2 for Punctuation. A punctuation token therefore is always in the interval ${( 2,3 ]}$.
...
(@0@ "2" @0 0@ "2 2")
\end{lstlisting}
Applying Distance Zero Representation to the above Java snippet gives  \ho{If I understand correctly, this is not correct. There shouldn't be more than one zero of the same color. Also why isn't there a green zero?!?! If it is correct, then I didn't understand correctly and you need to clarify more}:
...
(@0@ "2" @0 0@ "2 2")
\end{lstlisting}
Applying Distance All Representer gives the following representations \ho{Also this is not correct. The value shouldn't change as the maximum does not change. Also the green numbers are wrong. Again, of it is correct and I did not understand correctly, you need to clarify more}: 
...
In code, important information such as keywords can often be found in the beginning of a line of code. Weight Sum First Representer adds all \chg{values type}{type values} or distance values, following the current position, giving the vector elements in the beginning a higher weight.\newline
...
\begin{figure}[H]
	\begin{center}
		...
		\end{axis}
		\end{tikzpicture}
		\caption{Average Fit per centroid per k, calculated using Java and TypeStructureRepresenter.}
	\end{center}
\end{figure}
...
The two dimensional visualisation can be found in Figure \ref{fig:exLangDiff}. The red and blue crosses represent the data points of the clusters of the clustered language, with the centroid as the centre and the green crosses being the data points of the second language:\ho{So?}
\begin{figure}[h!]
	\centering
...
\section{Technology}
The k-means clustering tool is developed in Pharo 4.0\ho{provide a link as a footnote or a citation}. Statements and tokens are separated with Petit Parser \cite{bergel2013deep}, which is also used for recognizing the types of the tokens.
...
\begin{figure}[H]
	...
	\caption{Simplified data flow chart of the expanded \ho{with what? highlight the difference} k-means pipeline}
	\label{fig:bestkmens}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Simplified data flow chart of the Language Differentiator pipeline\ho{this diagram is confusing. Please find a better way to represent the process of clustering languages}}
	\label{fig:langdiff}
\end{figure}
\section{Runtime}
The runtime\ho{What do you mean by runtime? I don't understand. If you mean Execution, the just write Execution because runtime is something else \url{http://scg.unibe.ch/wiki/howtos/commonwritingerrors}} of the developed tool is dependent on different parameters of the input and configuration.
The parameters the tool depends on, are the number of iteration\ins{s}, the size of the training\del{s} data\chg{ and}{,} the number\del{s} of clusters, as well as the number of attributes, meaning the length\del{s} of the vector representations.

While running the algorithm, we experienced significant slow downs for calculating the clusterings, using the Type Structure Representer or with calculation using the Weight Inverse Representer. \chg{This}{these} two representations are the only ones using float calculation, instead of integer calculations. Because there is a lot of vector calculations involved, it leads to the conclusion that floating point calculation is the cause of the slowdowns.
\section{Testing}
...
\end{centering}
This gives us values for h, c and v ranging from $[0,1]$. \chg{This}{These} values can be used as a score to compare how well our representations performed, with 1 being a perfect score and 0 being the lowest score \cite{rosenberg2007v}.
\subsection{V-Measure scores for Java}\label{sec:v-measure-scores-for-java}
Using the v-measure scores, we can calculate two tables containing all the possible representations introduced in this thesis. The first table is clustered using the Type Structure representation, while the second table is calculated using Type Specific representation. The two tables are structured in a way, that first all the Representers are evaluated with the respective Type Representer and in an additional step, the Weight Representers are concatenated with the Distance Representers.

\begin{table}[H]
	\begin{center}
		\caption{Java v-measures of all possible representations with Type Structure Representer \ho{This table has some mess. What are these commas? What are the empty Dist All and Dist Zero rows?}}
		\label{table1}
...
A problem in this explanation lies therein, that empty centroids are not a problem for all the representations in the same way. Hence, there needs to be another problem causing this. Another look into the scores table reveals, that empty centroids are more common for Representers, that have higher variance in the data, as discussed under variance. \ho{So? What is the another explanation}\\\\
...
\ho{Add a summary paragraph suggesting what representation to use as the best.}
...
Also, there are two almost identical statements on line 1 and line 4\ho{where}, with the difference being in the length. This means, that the length of the representation does play a big part in the assignments of the statements to clusters. It is to note here, that this would be the same statement, if Weight Summary Representer would have been used with the Type Structure Representer.
\begin{figure}[H]
	\centering
	\label{fig:typespec}
...
\begin{table}[H]
	...
	\end{center}
\end{table}
If we look at the taxonomy of programming languages, we see that C\chg{ Sharp}{\#} is \del{a} very close \del{descended} \chg{of}{to} Java. Therefore, it is to be expected that many structural elements are the same for both languages. It \chg{does}{is} not\ins{ a} surprise that the results of our methods are not that different to those calculated for Java. The heuristic for k is not as good as for Java. While we manually classified C\chg{ Sharp}{\#} into 17 clusters, even the best guess missed two clusters. This makes \chg{for C Sharp $\alpha = -3$ not the best choice}{$\alpha = -3$ not the best choice for C\#}. Nevertheless, \chg{are the scores}{the scores} for the single use of the Type Representers \del{very}\ins{are} satisfying with the best v-measure around $0.8$. But it stands out, that the Weight Summary Representer - our noise reduction - does not perform as good as in Java. This is surprising, given the close relation between Java and  C\#. While it resulted in good scores for Java, giving the best score with Type Specific representation, the scores for C\# are remarkably low. Differences can lie in the composition of the used data. For this thesis around 300 lines of code for each language are used in the clustering of the language. A big challenge in clustering is the correct assignment of comments. If a code contains lots of comments that are formatted in different ways or even software code that is commented out, this will be clustered differently \chg{as}{from} intended. \del{This, as we want all comments to be assigned to the same cluster.} The C\# code used for clustering shows indeed lots of different comments, that were not clustered as intended.
...
Python does have similar constructs as Java, but is not as closely related to it compared to C++ or C\#. The evaluation of Python shows reasonable clusterings for Type Structure and Type Specific, but does also not well, if Weight Summary representation is applied. If we include the guessed value for k and the number of centroids, Type Structure does better than Type Specific representation, even though the Type Specific alone does have a higher score. \del{But}Unfortunately, k is not guessed as \chg{good}{well} for Type Structure as for Type Specific. The number of clusters in our manually created clustering is 12. Elbowing does achieve a good value for the Type Structure Representer, with or without the application of Weight Summary Representer, creating only one extra cluster. For Type Specific, \ugh{k is guessed with a difference of 4, respectively 6 extra centroids}\ho{I don't understand}. If we take a look at the used code, we see lots of different statements beginning with "print" and should be assigned to the same cluster. Having just one word at the beginning of the statement characterising the whole statement works the same way as comments do. This makes it hard for the clustering algorithm to assign them properly.
\section{XML}
\begin{table}[H]
	\begin{center}
...
All the previously evaluated programming languages have similar constructs. This cannot be said about XML, as it is a hierarchical mark-up language. The best results were scored with the Type Structure Representer, scoring high in homogeneity and completeness. The other results are not \del{absolutely} conclusive, because it cannot be determined, if noise reduction does help or not.\\\\
...
\ho{Here, also add that you did only one file per language. This is too little for making conclusions about what to do and not to do. You results are purely experimental and no statistical claims can be made at this point.}

\section{Type Structure vs. Type Specific}\label{sec:type-structure-vs-type-specific}
The two Type Representers are performing reasonably well. In this section we want to find out, which of the two, is expected to perform better.
...
It is hard to see, which one of the two Type Representers does overall perform better, as their performance \chg{lies heavy}{relies heavily} on the input. This also stands out if we calculate the averages of all the scores of the Type Structure and Type Specific representation. The average of all scores of the Type Structure Representer is $0.57$, while the average of all the Type Specific representation lies by $0.58$. But if we look at the average of the best performed representations per language, we get a score of $0.64$, and if we ignore Brainfuck, which we were not good in clustering it at all, we get a score of $0.71$. This makes it hard to tell, which of the two Representers is overall expected to perform better, as scores depend more on input language as on the Representer. Predictions based on the languages are very hard. As we saw in this chapter, even closely related languages can behave very differently in the clustering process.
...
Apart from the results given by the Type Representers and/or Weight Summary representation, the other representations did not\ins{ do} well in the evaluation, having low scores and were not robust regarding their computations, having high variance in multiple runs and resulting in empty centroids.
...
The \chg{run time}{execution} of the k-means implementation is quite \chg{high}{expensive} for big data sets and in particular, if the vectors contain floating point numbers. Better implementation of vector computations could certainly improve the run time of our tool.
